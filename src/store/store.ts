...
import { createSelectorHooks, SelectorHook } from 'zustand-react'
import { shallow } from 'zustand/shallow'
import { subscribeWithSelector as subscribeWithSelectorMiddleware } from 'zustand/middleware'
...
export function createStore<TState extends object, TActions extends object>(
  ...
  // Add subscribe with selector
  storeCreator = subscribeWithSelectorMiddleware(storeCreator)
  ...
  if (typeof window !== 'undefined' && onDestroy) {
    const originalCleanup = window.addEventListener('beforeunload', onDestroy)
    return store
  }
  ...
  actions: (set, get) => ({
    ...
    addNotification: (notification) => {
      ...
      if (notification.duration) {
        setTimeout(() => {
          const current = get()
          const notif = current.notifications.find(n => n.message === notification.message)
          if (notif) {
